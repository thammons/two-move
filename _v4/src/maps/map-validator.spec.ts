...
describe("map-validator", () => {
    ...
    test("returns undefined if map is empty", () => {
        const invalidMap = {} as IMap;
    ...
    test("returns undefined if player overlaps any walls", () => {
        const invalidMap = Utils.cloneMap(mapSetting);
        invalidMap.mapItems.get("player")![0].location =
            invalidMap.mapItems.get("wall")![0].location;
    ...
    test("returns undefined if goal overlaps any walls", () => {
        const invalidMap = Utils.cloneMap(mapSetting);
        invalidMap.mapItems.get("goal")![0].location =
            invalidMap.mapItems.get("wall")![0].location;
...
describe("check for collisions", () => {
    ...
    test('the next move is not valid when the turn direction is invalid (turn: "invalid"', () => {
        const newMap = Utils.cloneMap(map);
        newMap.mapItems.get("player")![0].direction = "invalid";

        const result = Validator.isNextMoveValid(newMap, 1);
    ...
    test("isNextMoveWall returns true when next move is a wall", () => {
        const result = Validator.isLocationWall(map, 20);
    ...
    test("isNextMoveWall returns false when next move is not a wall", () => {
        const result = Validator.isLocationWall(map, 1);
    ...
    test("no collision, player east, no attributes", () => {
        const newMap = Utils.cloneMap(map);
        newMap.mapItems.get("player")![0].direction = "east";
        newMap.mapItems.get("player")![0].location = 1;
    ...
    test("invalid direction, player direction 'invalid', no attributes", () => {
        const newMap = Utils.cloneMap(map);
        newMap.mapItems.get("player")![0].direction = "invalid";
    ...
    test("valid direction, player direction 'north', no step, no attributes", () => {
        const newMap = Utils.cloneMap(map);
        //TODO: should the function be checking the next direction is valid for
        //  the direction map?
        newMap.mapItems.get("player")![0].direction = "north";
        ...
        const result = Validator.checkPlayerCollision(newMap, false);
